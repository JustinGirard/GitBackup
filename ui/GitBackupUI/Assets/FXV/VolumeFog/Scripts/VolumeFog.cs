...
namespace FXV
{
    ...
    public class VolumeFog : MonoBehaviour
    {
        ...
        public float  lightScatteringFactor = 1.0f;

        [SerializeField, Range(0.0f, 1.0f)]
        public float lightReflectivity = 1.0f;
        ...
        internal bool RenderCameraPlane(Camera camera)
        {
            ...
            {
                //TODO: optimize this
                float nearOffset = camera.nearClipPlane / Mathf.Cos(camera.fieldOfView * 0.5f * Mathf.Deg2Rad);
            ...
            else
            {
                ...
                b.extents += Vector3.one * (camera.nearClipPlane / Mathf.Cos(camera.fieldOfView * 0.5f * Mathf.Deg2Rad));
        ...
        void UpdateMesh(Camera camera)
        {
            ...
            float stereoSeparationX = (camera.stereoEnabled || XR_ENABLED) ? 3.0f : 0.2f;
            float stereoSeparationY = (camera.stereoEnabled || XR_ENABLED) ? 1.0f : 0.2f;

            vertices = new Vector3[4]
            {
                camera.ViewportToWorldPoint( new Vector3(-stereoSeparationX, -stereoSeparationY, camera.nearClipPlane + zOffset) ),
                camera.ViewportToWorldPoint( new Vector3(1.0f + stereoSeparationX, -stereoSeparationY, camera.nearClipPlane + zOffset) ),
                camera.ViewportToWorldPoint( new Vector3(-stereoSeparationX, 1.0f + stereoSeparationY, camera.nearClipPlane + zOffset) ),
                camera.ViewportToWorldPoint( new Vector3(1.0f+stereoSeparationX, 1.0f + stereoSeparationY, camera.nearClipPlane + zOffset) )
            };
        ...
        internal static Material GetCachedMaterial(VolumeFog fogObject)
        {
            ...
            string cachePath = "MaterialsCache/" + materialKey;
            mat = Resources.Load(cachePath) as Material;
        ...
        internal void _DelayedSaveToCache(string materialKey, Material mat)
        {
            ...
            string saveFolder = Internal.fxvFogAssetConfig.AssetPath + "/Resources/MaterialsCache/";
